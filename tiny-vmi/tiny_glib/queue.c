...
/**
 * Double-ended Queue ported from GLib.
 * Only ported necessary functions;
 * Functions tailed to minimize the call stack with simplified logic.
 * 
*/
/**
 * SECTION:queue
...
g_queue_free (GQueue *queue)
{
  ...
  tiny_slice_free (sizeof (GQueue), queue);
}
...
g_queue_init (GQueue *queue)
{
  ...
  dbprint(VMI_DEBUG_TEST, "%s: queue not NULL\n", __FUNCTION__);

  queue->head = NULL;

  dbprint(VMI_DEBUG_TEST, "%s: head init to NULL\n", __FUNCTION__);
