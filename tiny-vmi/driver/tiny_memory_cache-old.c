...
static inline mem_cache_t create_new_mem_cache(int capacity){
    ...
    mem_cache_t mem_c = (mem_cache_t) malloc(sizeof(struct mem_cache_array));
	
	mem_cache_line_t mem_cache_lines=(mem_cache_line_t *)malloc(capacity*sizeof(mem_cache_line_t));
	
	mem_c->cache_lines=mem_cache_lines;
    mem_c->count=0;
    // mem_c->capacity = capacity;
...
static int get_key_mem_index(mem_cache_t mem_c, mem_key_t key){
	int i,found=0;
	//dbprint(VMI_DEBUG_MEMCACHE, "--searching the mem key:<paddr>=<0x%lx>, in %s\n",*key,__FUNCTION__);
	if(mem_c->count<=0){
	//	dbprint(VMI_DEBUG_MEMCACHE, "--nothing in mem cache, in %s\n",__FUNCTION__);
		return -1;
	}
	...
	//	dbprint(VMI_DEBUG_MEMCACHE, "---hit the key:%d<paddr>=<0x%lx>, in %s\n",i,*(mem_c->cache_lines[i]->key),__FUNCTION__);
			found=1;
			return i;
		}
	}
	...
	//dbprint(VMI_DEBUG_MEMCACHE, "---no key match 0x%lxin %s\n",*key,__FUNCTION__);
	return -1;}
}
...
mem_cache_entry_t mem_cache_lookup(mem_cache_t mem_c,mem_key_t key){
	...
	int key_i;
	//dbprint(VMI_DEBUG_MEMCACHE, "---searching the key:<paddr>=<0x%lx>, in %s\n",*key,__FUNCTION__);
	key_i=get_key_mem_index(mem_c,key);
...
status_t mem_cache_insert(mem_cache_t mem_c,mem_key_t key,mem_cache_entry_t entry){
	 ...
	 if(mem_c->count<MAX_MEM_CACHE){
		...
		//dbprint(VMI_DEBUG_MEMCACHE, "---mem key set:line[%d],paddr=0x%lx, in %s\n",mem_c->count,*key,__FUNCTION__);
		mem_c->cache_lines[mem_c->count]=(mem_cache_line_t)malloc(sizeof(struct mem_cache_line));
		//dbprint(VMI_DEBUG_MEMCACHE, "---mem key set:line[%d],allocated, in %s\n",mem_c->count,__FUNCTION__);
		
		mem_c->cache_lines[mem_c->count]->key=key;
		//dbprint(VMI_DEBUG_MEMCACHE, "---mem key set:line[%d],set key=0x%lx, in %s\n",mem_c->count,*key,__FUNCTION__);
		mem_c->cache_lines[mem_c->count]->entry=entry;
		//dbprint(VMI_DEBUG_MEMCACHE, "---mem key set:line[%d],set entry(paddr=0x%lx), in %s\n",mem_c->count,entry->paddr,__FUNCTION__);
		
		mem_c->count++;
		//dbprint(VMI_DEBUG_MEMCACHE, "---mem set (count=%d), in %s\n",mem_c->count,__FUNCTION__);
		res = VMI_SUCCESS;
	}else{
        dbprint(VMI_DEBUG_MEMCACHE, "TODO: need to find a way to remove the least used cache lines; in %s, cache size: %d; \n",
            mem_c->count,__FUNCTION__);
...
static inline tiny_list_t create_new_list(int capacity){
...
//implementation
 ...
 void
mem_cache_entry_free(vmi_instance_t vmi, gpointer data)
{
 ...
 void * get_memory_data(vmi_instance_t vmi,
    addr_t paddr,
    uint32_t length)
{
...
//  void remove_entry(mem_key_t key, mem_cache_t cache)
// {
...
*/
    ...
    if (!mem_c){
        ...
        for(i=0;i<mem_c->count;i++){
            ...
            mem_cache_entry_free(vmi, mem_c->cache_lines[i]->entry);
        ...
        uint32_t length)
{
   ...
   //     dbprint(VMI_DEBUG_MEMCACHE, "--requesting PA [0x%"PRIx64"] beyond memsize [0x%"PRIx64"]\n",
   //             paddr + length, vmi->size);
   //     dbprint(VMI_DEBUG_MEMCACHE, "\tpaddr: %"PRIx64", length %"PRIx32", vmi->size %"PRIx64"\n", paddr, length,
   //             vmi->size);
   //     return 0;
   // }
    dbprint(VMI_DEBUG_MEMCACHE, "->-> now in %s\n", __FUNCTION__);
...
void memory_cache_init(
    ...
    mem_cache_t mem_c = create_new_mem_cache(MAX_MEM_CACHE);
    vmi->memory_cache = mem_c;
	...
	//if(vmi->memory_cache_lru->head==NULL)	     dbprint(VMI_DEBUG_MEMCACHE, "head=tail=NULL\n");
	//else      dbprint(VMI_DEBUG_MEMCACHE, "error, head not NULL\n");
    vmi->memory_cache_age = age_limit;
    // vmi->memory_cache_size = 0;
    // vmi->memory_cache_size_max = MAX_PAGE_CACHE_SIZE;
    vmi->memory_cache_size_max = MAX_MEM_CACHE;
    ...
    addr_t paddr)
{
    ...
    if (paddr != paddr_aligned) {
        dbprint(VMI_DEBUG_MEMCACHE, "Memory cache request for non-aligned page\n");
        return NULL;
    }
	//dbprint(VMI_DEBUG_MEMCACHE, "->-> now in %s\n",__FUNCTION__);
    mem_key_t key = &paddr;
    if ((entry = mem_cache_lookup(vmi->memory_cache, key)) != NULL) {
        ...
        if (!entry) {
         ...
         //   dbprint(VMI_DEBUG_MEMCACHE, "create_new_entry for %lx,in %s\n",paddr,__FUNCTION__);
		//}
        ...
        *key2 = paddr;
		...
		//dbprint(VMI_DEBUG_MEMCACHE, "---key2 for list=0x%lx, in %s\n",*key2,__FUNCTION__);
