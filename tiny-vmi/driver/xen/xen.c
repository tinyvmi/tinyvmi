    ...
    int prot)
{
    ...
    dbprint(VMI_DEBUG_XEN, "%s: done map foreign range\n", __FUNCTION__);
    ...
    const char *name)
{
    ...
    dbprint(VMI_DEBUG_XEN, "xs_directory returns: %s\n", domains);

    if (!domains){
        dbprint(VMI_DEBUG_XEN, "invalide domain info from xs_directory, (returns: %s)\n", domains);
        goto _bail;
    }
    ...
    void *UNUSED(init_data))
{
    ...
    if ( VMI_FAILURE == create_libxs_wrapper(xen) )
    {
        ...
        dbprint(VMI_DEBUG_XEN, "libxs wrapper created\n");
    }
    ...
    vmi_instance_t vmi)
{
    ...
    dbprint(VMI_DEBUG_XEN, "%s:\n", __FUNCTION__);
    dbprint(VMI_DEBUG_XEN, "\t vm_type : %d\n", vmi->vm_type);
    ...
    // xc_interface *xchandle = xen->xchandle;

    if ( xen->xchandle ){

        if (!xen->libxcw.xc_interface_close){
            dbprint(VMI_DEBUG_XEN, "%s: xc_interface_close not defined\n", __FUNCTION__);
            return;
        }else{
            dbprint(VMI_DEBUG_XEN, "%s: close xchandle\n", __FUNCTION__);
            xen->libxcw.xc_interface_close(xen->xchandle);
...
#ifdef HAVE_LIBXENSTORE
    if(xen->xshandle) {
        dbprint(VMI_DEBUG_XEN, "%s: close xshandle\n", __FUNCTION__);
    ...
    const char *name)
{
    ...
    if (domainid == VMI_INVALID_DOMID) { /* name != NULL */
        ...
        if (domainid == VMI_INVALID_DOMID) {
            dbprint(VMI_DEBUG_XEN, "%s: failed to find domainid from name, now destroy xen instance\n", __FUNCTION__);
            xen_destroy(vmi);
    ...
    if ( VMI_FAILURE == xen_check_domainid(vmi, domainid) ) {
        ...
        dbprint(VMI_DEBUG_XEN, "%s: failed to check domainid\n", __FUNCTION__);
        dbprint(VMI_DEBUG_XEN, "\n%s: ********** might need human interaction **************\n", __FUNCTION__);
        dbprint(VMI_DEBUG_XEN, "%s: ********** default domain id can be set at **************\n", __FUNCTION__);
        dbprint(VMI_DEBUG_XEN, "%s: ********** tinvmi/include/tiny_private.h **************\n\n", __FUNCTION__);

        dbprint(VMI_DEBUG_XEN, "%s: now destroy xen instance\n", __FUNCTION__);
